...
//取出产品二级目录
$strSQL = "select id_proddir,name from productdir where dele=1 and fatherid=1 order by ordernum asc " ;
...
$strSQL = "select a.*,b.fatherid from productinfo as a left join productdir as b on a.id_proddir=b.id_proddir  where a.id_proddir='$_GET[id2]' and a.dele=1 order by a.id_prodinfo desc" ;
...
$strSQLNum = "SELECT COUNT(*) as num from productinfo as a left join productdir as b on a.id_proddir=b.id_proddir   where a.dele=1 and b.fatherid=1";   
$rs = $objDB->Execute($strSQLNum);
$arrNum = $objDB->fields();
$intTotalNum=$arrNum["num"];
...
$strSQL = "select a.*,b.lang,b.fatherid from productinfo as a left join productdir as b on a.id_proddir=b.id_proddir   where a.dele=1 and b.fatherid=1 order by a.id_prodinfo desc" ;
$objDB->SelectLimit($strSQL,$intRows,$intStartNum);
$arrprods=$objDB->GetRows();
