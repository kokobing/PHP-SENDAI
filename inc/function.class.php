<?php
//版本日期：2012-06-30
if($_SERVER['HTTP_HOST']=='demo-cs2.vicp.net:8080' || $_SERVER['HTTP_HOST']=='demo-cs10.vicp.net'){$webcoming=1;}else{$webcoming=0;}
...
$strSQL="SELECT id_proddir,name,intro FROM productdir WHERE level='2' and dele='1' and fatherid='".$fatherid."' order by ordernum desc";
$objDB->Execute($strSQL);
...
function getproductinfo($pageid,$tableField){
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select $tableField from productinfo  where dele=1 and id_prodinfo=$pageid  " ;
$objDB->Execute($strSQL);
...
function getnewslist($newsid,$tableField){
global	 $objDB;//函数调用 声明全局变量
$strSQL = "select $tableField from newsinfo where dele=1 and id_newsdir=$newsid  order by ordernum desc " ;
$objDB->Execute($strSQL);
...
//取出区块pagesetpic指定id的所有图片信息 指定数据库字段tableField 的值  多条  返回变量数组 引用时需要赋值 再输出
function getallpagesetpicnuminfo($lid,$tableField){
global	 $objDB;//函数调用 声明全局变量
